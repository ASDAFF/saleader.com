        ...
        <div id="tableContainer">
            ...
            <div id="elementContainer" class="column">
                <div class="mainContainer" id="browse">
                    ...
                    <div
                        class="col<? if (empty($arResult["PREVIEW_TEXT"]) && empty($arResult["SKU_PRODUCT"]) && empty($arResult["DISPLAY_PROPERTIES"])): ?> hide<? endif; ?>">
                        <noindex>
                            <div id="smallElementTools">
                                <? include($_SERVER["DOCUMENT_ROOT"] . "/" . $templateFolder . "/include/right_section.php"); ?>
                            </div>
                            <? if (!empty($arResult["BRAND"]["PICTURE"])): ?>
                                <a rel="nofollow" href="<?= $arResult["BRAND"]["DETAIL_PAGE_URL"] ?>"
                                   class="brandImage"><img src="<?= $arResult["BRAND"]["PICTURE"]["src"] ?>"
                                    ...
                                    <? foreach ($arResult["SKU_PROPERTIES"] as $propName => $arNextProp): ?>
                                        <? if (!empty($arNextProp["VALUES"])): ?>
                                            <div class="elementSkuProperty" data-name="<?= $propName ?>"
                                                 data-level="<?= $level++ ?>">
                                                <div class="elementSkuPropertyName"><?= preg_replace("/\[.*\]/",'',trim($arNextProp["NAME"])) ?>:</div>
                                                <ul class="elementSkuPropertyList">
                                                    <? foreach ($arNextProp["VALUES"] as $xml_id => $arNextPropValue): ?>
                                                        <li class="elementSkuPropertyValue<? if ($arNextPropValue["DISABLED"] == "Y"): ?> disabled<? elseif ($arNextPropValue["SELECTED"] == "Y"): ?> selected<? endif; ?>"
                                                            ...
                                                            <a rel="nofollow" href="#" class="elementSkuPropertyLink">
                                ...
                                <div class="elementProperties changePropertiesNoGroup">
                                    <div class="headingBox">
                                        ...
                                        <div class="moreProperties">
                                            <a rel="nofollow" href="#"
                                               class="morePropertiesLink"><?= GetMessage("CATALOG_ELEMENT_MORE_PROPERTIES") ?></a>
                                        </div>
                                    </div>
                                        ...
                                        <? foreach ($arResult["DISPLAY_PROPERTIES"] as $ip => $arProperty):?>
                                            <? if (!empty($arProperty["DISPLAY_VALUE"]) && ++$propertyCounter <= $countPropertyElements): ?>
                                                <div class="propertyTable">
                                                    <div
                                                        class="propertyName"><?= preg_replace("/\[.*\]/", "", $arProperty["NAME"]) ?></div>
                        ...
                        <table class="stats">
                            ...
                            <? if (!empty($arSome)): ?>
                                ...
                                <? foreach ($arSome as $arProp): ?>
                                    ...
                                    <tr<? if ($set % 2): ?> class="gray"<? endif; ?>>
                                        <td class="name">
                                            <span><?= $arProp["NAME"] ?></span><? if (!empty($arProp["HINT"])): ?><a
                                                href="#" class="question" title="<?= $arProp["HINT"] ?>"
                                                data-description="<?= $arProp["HINT"] ?>"></a><? endif; ?></td>
                                        <td>
                                            <? if ($arProp["PROPERTY_TYPE"] == "E" || $arProp["PROPERTY_TYPE"] == "S"): ?>
                                                <?= $arProp["DISPLAY_VALUE"] ?>
                ...
                <? if (isset($arResult["REVIEWS"])): ?>
                    <div id="catalogReviews">
                        <div class="heading"><?= GetMessage("REVIEW") ?> (<?= count($arResult["REVIEWS"]) ?>
                            ) <? if ($arParams["SHOW_REVIEW_FORM"]): ?><a href="#"
                                                                          class="reviewAddButton"><?= GetMessage("REVIEWS_ADD") ?></a><? endif; ?>
                            <div class="ratingContainer">
                                <div class="label"><?= GetMessage("RATING_PRODUCT") ?> </div>
                                <div class="rating"><i class="m"
                                                       style="width:<?= ($arResult["PROPERTIES"]["RATING"]["VALUE"] * 100 / 5) ?>%"></i><i
                                        class="h"></i></div>
                            </div>
                        </div>
                        <ul id="reviews">
                                    ...
                                    <div class="reviewTable">
                                        <div class="reviewColumn">
                                            <div class="reviewDate">
                                                <div
                                                    class="label"><?= GetMessage("REVIEWS_DATE") ?></div> <?= FormatDate(array(
                                                    "tommorow" => "tommorow",
                                                    "today" => "today",
                                                    "yesterday" => "yesterday",
                                                    "d" => 'j F',
                                            ...
                                            <div class="reviewName">
                                                <div
                                                    class="label"><?= GetMessage("REVIEWS_AUTHOR") ?></div> <?= $arReview["PROPERTY_NAME_VALUE"] ?>
                                            </div>
                                            <div class="reviewRating">
                                                <span class="rating"><i class="m"
                                                                        style="width:<?= ($arReview["PROPERTY_RATING_VALUE"] * 100 / 5) ?>%"></i><i
                                                        class="h"></i></span>
                                            </div>
                                        </div>
                                        <div class="reviewColumn">
                                        ...
                                        <div class="tb">
                                            <div class="tbr">
                                                <div class="icon">
                                                    <a href="<?= $arFile["SRC"] ?>">
                                                        <img
                                                            src="<?= SITE_TEMPLATE_PATH ?>/images/file<?= $fileType ?>.png"
                                                            alt="<?= $arFile["PARENT_NAME"] ?>">
                                                    </a>
                                                </div>
                                                <div class="info">
                                                    <a href="<?= $arFile["SRC"] ?>"
                                                       class="name"><span><?= $arFile["ORIGINAL_NAME"] ?></span></a>
                                                    <small
                                                        class="parentName"><?= preg_replace("/\[.*\]/", "", trim($arFile["PARENT_NAME"])) ?>
