...
<?if(!empty($arResult)):?>
	<div id="appFastView"<?if(!empty($arResult["SKU_ACTIVE_OFFER"])):?> class="sku"<?endif;?> data-product-id="<?=!empty($arResult["~ID"]) ? $arResult["~ID"] : $arResult["ID"]?>"<?if(!empty($arResult["SKU_INFO"])):?> data-iblock-id="<?=$arResult["SKU_INFO"]["IBLOCK_ID"]?>" data-prop-id="<?=$arResult["SKU_INFO"]["SKU_PROPERTY_ID"]?>"<?endif;?> data-product-width="<?=$arParams["PICTURE_WIDTH"]?>" data-product-height="<?=$arParams["PICTURE_HEIGHT"]?>" data-hide-measure="<?=$arParams["HIDE_MEASURES"]?>" data-cast-func="fastViewSku" data-change-prop="fast-view" data-more-pictures="Y">
			...
			<div class="appFastViewColumnContainer">
				...
				<div class="appFastViewInformationColumn">
					...
					<div class="secondTool">
						...
						<?endif;?>	
						<div class="row">
							<a href="#" class="fastBack label changeID" data-id="<?=$arResult["ID"]?>"><img src="<?=SITE_TEMPLATE_PATH?>/images/fastBack.png" alt="<?=GetMessage("FAST_VIEW_FASTBACK_LABEL")?>" class="icon"><?=GetMessage("FAST_VIEW_FASTBACK_LABEL")?></a>
						</div>
						<div class="row">
