                    ...
                    <?if((!empty($arProp["VALUE"]) || !empty($arProp["LINK"])) && $arProp["SORT"] <= 5000):?>
                        ...
                        <?if(preg_match("/\[.*\]/", trim($arProp["NAME"]), $res, PREG_OFFSET_CAPTURE)):?>
                            ...
                            <tr<?if($set % 2):?> class="gray"<?endif;?>>
                                ...
                                <td class="right">
                                    <?if($arProp["FILTRABLE"] =="Y" && !is_array($arProp["VALUE"]) && !empty($arResult["LAST_SECTION"])):?><a href="<?=$arResult["LAST_SECTION"]["SECTION_PAGE_URL"]?>filter/<?=strtolower($arProperty["CODE"])?>-is-<?=strtolower($arProperty["VALUE_XML_ID"])?>/apply/" class="analog"><?=GetMessage("OTHERITEMS")?></a><?endif;?>
                                </td>
                            </tr>
                        <?else:?>
                            <? $arSome[] = $arProp?> 
                        ...
                        <tr<?if($set % 2 ):?> class="gray"<?endif;?>>
                            ...
                            <td class="right">
                                <?if($arProp["FILTRABLE"] =="Y" && !is_array($arProp["VALUE"]) && !empty($arProp["VALUE_ENUM_ID"]) && !empty($arResult["LAST_SECTION"])):?>
                                    <a href="<?=$arResult["LAST_SECTION"]["SECTION_PAGE_URL"]?>filter/<?=strtolower($arProperty["CODE"])?>-is-<?=strtolower($arProperty["VALUE_XML_ID"])?>/apply/" class="analog"><?=GetMessage("OTHERITEMS")?></a>
                                <?endif;?>
                            </td>
                        </tr>
                    <?endforeach;?>
